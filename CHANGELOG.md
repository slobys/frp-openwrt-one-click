...
- 重构菜单为两级结构
- 一级菜单：在服务器上安装 / 在软路由上安装
- 新增服务器端安装/管理（systemd + iptables）
- 软路由脚本重命名为 `*-openwrt.sh`

## 1.4.1

- 新增 `config.sh` 面板账号修改脚本
- 菜单新增"修改面板账号/密码"功能，可修改 frps/frpc 面板用户名、密码和 token
- 修改后自动重启对应服务
...
- 避免 `frp` 继续使用旧版 `install.sh` 导致旧逻辑残留

## 1.3.5

- 撤销安装 frps 后自动放行 `60000-60999` 的逻辑，避免误开放公网端口
- 防火墙端口放行改为仅通过菜单手动执行
