...
FRPS_DASHBOARD_PASSWORD="${FRPS_DASHBOARD_PASSWORD:-naiyou_admin_2026}"
FRP_TOKEN="${FRP_TOKEN:-naiyou_frp_2026_please_change_this_token}"
...
FRPC_DASHBOARD_USER="${FRPC_DASHBOARD_USER:-admin}"
FRPC_DASHBOARD_PASSWORD="${FRPC_DASHBOARD_PASSWORD:-naiyou_frpc_2026}"
...
print_summary() {
    ...
    echo
    log "安装完成"
    [ "$INSTALL_MODE" = "both" ] || [ "$INSTALL_MODE" = "frps" ] && {
        echo "frps Dashboard: http://软路由IP:${FRPS_DASHBOARD_PORT}"
        echo "frps 账号: ${FRPS_DASHBOARD_USER}"
        echo "frps 密码: ${FRPS_DASHBOARD_PASSWORD}"
    }
    [ "$INSTALL_MODE" = "both" ] || [ "$INSTALL_MODE" = "frpc" ] && {
        echo "frpc Dashboard: http://${lan_ip}:${FRPC_DASHBOARD_PORT}"
        echo "frpc 账号: ${FRPC_DASHBOARD_USER}"
...
main() {
    ...
    local arch lan_ip need_frps need_frpc
    arch="$(detect_arch)"
    lan_ip="$(get_lan_ip)"
    need_frps="0"
    need_frpc="0"
